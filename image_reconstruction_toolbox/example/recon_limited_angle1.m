...
if ~isvar('R'), printm 'R'
	R = Robject(ig.mask, 'type_denom', 'matlab', 'beta', 2^9, ...
		'potential', 'hyper3', 'delta', 1);
%		'potential', 'quad');
...
% todo: something below needs work!
if 0 & ~isvar('xiot'), printm 'xiot'
	f.niter = 3;

	Gb = Gblock(G, 26);
	xiot = pl_iot(xinit(ig.mask), Gb, {sino, ones(size(sino))}, R, ...
		'dercurv', 'wls', ...
		'riter', 2, ...
		'os', 5, ... % f.niter, ...
		'niter', f.niter, ...
