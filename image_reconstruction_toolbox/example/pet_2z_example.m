...
%
% Copyright 2005-6-13, Jeff Fessler, The University of Michigan
...
if ~isvar('G'), printm 'G'
	...
	f.wtr = strrep(f.dsc, 'dsc', 'wtf');
	f.mask = [test_dir 'mask.fld'];
	fld_write(f.mask, ig.mask)

	sg = sino_geom('par', 'nb', ig.nx, 'na', 60 * ig.nx / 64, 'dr', ig.dx);

	t = 'wt -chat 0 dsc -support "file %s" 2 nx %d ny %d nb %d na %d pixel_size %g scale 0';
	t = sprintf(t, f.mask, ig.nx, ig.ny, sg.nb, sg.na, ig.dx)
	os_run([t ' >! ' f.dsc])
...
%
	...
	if ~isvar('R'), printm 'R'
		...
		if 0 % predicted PSF for helping choose beta
			wi = ci(:).^2 ./ max(yi(:),1);
			psf = qpwls_psf(G, R, 1, ig.mask, diag_sp(wi));
			im(psf, 'psf'), cbar
		return
		end, clear wi
	end
