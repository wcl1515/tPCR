 function st = nufft_init_fast_rm(om, Nd, Jd, Kd, varargin)
...
for id=1:dd
	...
	kd{id} = outer_sum([1:J]', koff') ;	% [J?,M] {1,...,K?}
    mask = and(kd{id}>=-floor(K/2),kd{id}<floor(K/2));

%%%
    if id==3
        tra_max=max(kd{id}(:));
        tra_min=min(kd{id}(:));
        st.zrange=mod((tra_min:tra_max),K)+1;
        znum=tra_max-tra_min+1;
        kd{id}=kd{id}-tra_min+1;
    else
...
st.p = sparse(mm(:), kk(:), uu(:), M, prod([Kd(1:2) znum]));	% sparse matrix
st.p = Gsparse(st.p, 'odim', [M 1], 'idim', [prod([Kd(1:2) znum]) 1]);
