 function st = gnufft_init(om, Nd, Jd, Kd, varargin)
%function st = gnufft_init(om, Nd, Jd, Kd, [n_shift,] ...)
%
...
st.p_real = gsingle(real(st.p));
st.p_real_adj = gsingle(real(st.p'));
st.p_imag = gsingle(imag(st.p));
st.p_imag_adj = gsingle(imag(st.p'));
