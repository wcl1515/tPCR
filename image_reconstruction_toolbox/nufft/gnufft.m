...
if ndims(x) == dd
	...
	Xk = reshape(gfftn(x),[prod(size(x)) 1]);  	% [*Kd] oversampled FFT, padded at end
...
if ~isvar('st.interp_table')
	X = ((st.p_real * real(Xk)) - (st.p_imag * imag(Xk)) + 1i * ( (st.p_imag * real(Xk)) + (st.p_real * imag(Xk))));					% [M,*L]
else
