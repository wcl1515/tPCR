...
if ~isvar('st.interp_table_adj')
    Xk_all = ((st.p_real_adj * real(X)) - (st.p_imag_adj * imag(X)) + 1i * ( (st.p_imag_adj * real(X)) + (st.p_real_adj * imag(X))));					% [M,*L]
else
...
for ll=1:Lprod
	...
	x(:,ll) = prod(Kd) * col(gifftn(Xk));	% scale factor!
end
