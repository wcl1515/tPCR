...
if ~isvar('xinit')
	...
	% test Fatrix too
	T = Fatrix(size(T), T, 'caller', mfilename);
	...
	printm('Cond before %g after %g', cond(T.arg), cond(T + C'*C))
	xh = (T + C'*C) \ bb;
...
if ~isvar('x1t')
	...
	M1 = 1;
	x1g = qpwls_pcg1(xinit, G, 1, yy, C, 'precon', M1, it{:});
...
if ~isvar('x2t')
	M2 = diag(1 ./ diag(T + C'*C));
	x2g = qpwls_pcg1(xinit, G, 1, yy, C, 'precon', M2, it{:});
...
if ~isvar('x3t')
	x3g = qpwls_pcg1(xinit, G, 1, yy, C, 'precon', M3, it{:});
...
if ~isvar('x4t') % test stopper
	...
	jf_assert isequal(x3t(:,1:ncol(x4t)), x4t)
end
