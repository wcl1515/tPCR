...
%
% Copyright July 1996	Jeff Fessler, The University of Michigan
...
if 1
	for ig = 1:ng
		gg = groups(:,ig);
		disp(sprintf('precompute %g, assuming nonnegative A', ig))
...
if chat & (np < 1000)
	...
	if min(t(:)) < 1-10*eps
		disp(['Under-relax. range:' sprintf(' %g', range(t))])
		t = embed(t,wlsden);
		im(324, embed(t,mask), 'Under-relaxer')
	else
		disp 'Note: GCA with ideal denominator!'
	end
end
...
for ig = 1:ng
	...
	t = range(t);
	if min(t) < 1-10*eps
		disp(['Virtual under-relax. range:' sprintf(' %g', t)])
