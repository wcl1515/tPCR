...
% generate data
%
if ~isvar('yi'), disp 'setup qpwls_qn_test'
	em_wls_test_setup
	W = diag_sp(wi(:));
prompt
end
...
%	im(223, (xart2-xmat)/max(xmat(:)), '(aspire-matlab)/max(matlab)')
	...
	t1 = pwls_cost(xsps, G, W, yi(:), R, ig.mask);
	t2 = pwls_cost(xpcg, G, W, yi(:), R, ig.mask);
	t3 = pwls_cost(xqnp, G, W, yi(:), R, ig.mask);
	t4 = pwls_cost(xqnu, G, W, yi(:), R, ig.mask);
	if im
		subplot(224)
