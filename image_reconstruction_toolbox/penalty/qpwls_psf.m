 function [psf, var, fwhm, mtf, con] = qpwls_psf(G, C, beta, mask, W, varargin)
%function [psf, var, fwhm, mtf, con] = qpwls_psf(G, C, beta, mask, W, [options])
%
% Compute FFT-approximate PSF
% for quadratically-penalized shift-invariant unweighted least-squares
% uses psf = [G'WG + beta C'C]^{-1} G'W y, where usually y = G e
%
...
if nargin == 1 && streq(G, 'test'), qpwls_psf_test; return, end
...
if ~islogical(mask), warning 'not logical mask', mask = logical(mask); end
...
% jth column of gram matrix: F ej = G'WG ej
if isempty(arg.gtwgej)
	...
	if 0
		...
		if any(dpad ~= dim)
			warning(['non power of 2, padding ', num2str(dpad)])
		end
	end

	% jth column of Fisher information: G'WG ej
	gtg = embed(G' * (W * (G * ej(mask(:)))), mask);
...
%
ctc = form_ctc(C, beta, ej, mask, size(gtg));
...
elseif ndim == 3
	...
	if any(arg.offset ~= 0)
		warning 'phase correction not implemented'
	end
	phi = 0;
end
...
if max(f.gtg(:)) <= 0
	warning 'spectrum of Fisher information must have nonzero maximum!'
	keyboard
end
if min(f.gtg(:)) < 0
	...
	f.gtg = max(f.gtg,0); % ensure FT(G'G) is >= 0 for inverse filter !!!
end
...
if any(f.hes(:) <= 0)
	warning(sprintf('%d zeros in Hessian - results may be meaningless', ...
		sum(f.hes(:) == 0)))
	f.hes(f.hes == 0) = inf;
end

if ~isempty(arg.yb)
	gtwy = embed(G' * (W * arg.yb(:)), mask);
	...
	psf = reale(fftshift(ifftn(f.psf)), arg.iwarn);
end
...
%	im(pl+1, ix, iy, abs(gtg), '|G''WG|'), cbar h
	im(pl+1, ix, iy, real(gtg), 'real(G''WG)'), cbar h
%	im(pl+2, ix, iy, imag(gtg), 'imag(G''WG)'), cbar h
	im(pl+2, ix, iy, ctc, 'C''C'), cbar h
	im(pl+3, ix, iy, psf, 'psf'), cbar h

	ix = [-dim(1)/2:dim(1)/2-1];
	iy = [-dim(2)/2:dim(2)/2-1];
	im(pl+4, ix, iy, fftshift(f.gtg), 'F(G''WG)'), cbar h
	im(pl+5, ix, iy, fftshift(f.ctc), 'F(C''C)'), cbar h
	im(pl+6, ix, iy, fftshift(f.hes), 'F(Hess)'), cbar h

	im(pl+7, ix, iy, fftshift(f.gtg) == 0, 'F(G''WG) == 0'), cbar h
	im(pl+8, ix, iy, fftshift(f.ctc) == 0, 'F(C''C) == 0'), cbar h
	f.hes(f.hes == inf) = 0;
	im(pl+9, ix, iy, fftshift(f.hes) == 0, 'F(Hess) == 0'), cbar h
end
...
%
function ctc = form_ctc_strum(R, ej, size_gtg)

try
	ctc = R.cgrad(R, 1e-2 * ej(R.mask(:))) / 1e-2; % trick
	ctc = embed(ctc, R.mask);
catch
...
% determine C' * C * ej
%
function ctc = form_ctc(C, beta, ej, mask, size_gtg)
...
elseif streq(class(C), 'strum') ... % new Reg1 strum
	...
	ctc = form_ctc_strum(C, ej, size_gtg);
...
if ndim == 2
	try
		...
		warning 'fwhm2 failed, returning NaN for fwhm'
		fwhm = NaN;
	end

elseif ndim == 3
	if ~arg.shift0 && any(arg.offset) && ~streq(arg.fwhmtype, 'fwhm3')
		warning('3d "fwhm" probably wrong without shift0 option')
	end
	...
	try
		...
		case 'slice' % old: report separate yz, xz, xy fwhm!
			...
			tmp = @(psf, i1, i2) ...
				fwhm2(sqeeze(psf), 'dx', dd(i1), 'dy', dd(i2));
			fwhm = [
...
%			im(psf(:,:,floor(end/2)-1+[-3:3])), cbar
		...
		warning 'fwhm(3d) failed, returning NaN for fwhm'
keyboard
		fwhm = NaN;
	end
...
[psf0 var fwhm mtf con0] = ...
	qpwls_psf(G, C, beta, ig.mask, [], 'offset', offset, 'chat', 1);
printm('approximate stddev = %g', sqrt(var))

if 0 % check matched yb version.
	ej = ig.unitv(ig.nx/2+1+offset(1), ig.ny/2+1+offset(2));
	yb1 = G * ej;
	psf1 = qpwls_psf(G, C, beta, ig.mask, [], 'offset', offset, ...
...
%	max_percent_diff(yb1, yb2)
	psf2 = qpwls_psf(G, C, beta, ig.mask, [], 'offset', offset, ...
		'chat', 0, 'shift0', 0, 'yb', yb2);
	max_percent_diff(psf0, psf2)
	im clf, im([psf0, psf2, psf0-psf2])
end
...
if 1 % test "R" version
	...
	psf3 = qpwls_psf(G, R3, 1., ig.mask, [], 'offset', offset, 'chat', 0);
	equivs(psf0, psf3)
end
