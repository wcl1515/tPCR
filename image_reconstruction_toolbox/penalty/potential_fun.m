...
% trick: for now these all require an extra argument for compat with old version
%	pot.potk(nan, C*x)
%
% Copyright 2004-5-18, Jeff Fessler, The University of Michigan
...
% trick: gf1-fit means match gf1 to named potential at given points [0, t1, t2]
% param: {'name of potential to fit', points, param}
if streq(type, 'gf1-fit')
	type = param{1};
	tv = col(param{2});
	param = param{3};
	pot = potential_fun(type, delta, param);
	pt = pot.wpot([], tv);
	param = [1 1; -pt']' \ ((pt-1) ./ tv);
	type = 'gf1';
	clear pt pot tv
end
...
% generalized fair 1st-order potential
...
if 0 & im % polynomial approximations to qgg2
	param = 1.2;
	delta = 1;
	t = linspace(-10,10,101);
	pot = potential_fun('qgg2', delta, param);
	pw = pot.wpot([], t);
	tv = [1 10]';
	pt = pot.wpot([], tv);
	coef = [1 1; -pt']' \ ((pt-1) ./ tv)
	pp = @(t) (1 + coef(1) * abs(t)) ./ (1 + coef(2)*abs(t));
	clf, plot(t, pw, '-', t, pp(t/delta), '--')

	if 0 % analyze corresponding pot function
		syms x
		syms a positive
		syms b positive
		syms t positive
