...
%	C = Csparse('maskleak', mask);		method = '@1@alg@0,quad,1,-'
	...
	disp(sprintf('like=%.11e penal=%.11e', ...
...
mask = logical(double(imdilate(x > 0, ones(3))));
%mask([1 end],:) = 0; mask(:,[1 end]) = 0;
%mask = true(size(x));
%mask = logical(double(imerode(ones(size(x)), ones(3))));
im(221, mask+x, 'mask + x')

kappa = 2 - double(edge(x, 'sobel', 0.06));
kappa = mask .* kappa;
im(222, kappa, 'kappa')

C = Csparse('b2info', kappa, mask, 0);
if im, subplot(223), spy(C), end
