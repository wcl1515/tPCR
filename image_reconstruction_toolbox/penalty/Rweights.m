...
%		wt.col(m)	return w[: ; m]
%
% Copyright 2006-12-2, Jeff Fessler, The University of Michigan
...
otherwise
	error('unknown type %s', arg.type_wt)
end
...
for order=1:2
 for ie=1:length(edge_types)
	...
	xlabel(sprintf('order=%d type=%s', order, edge_type))
