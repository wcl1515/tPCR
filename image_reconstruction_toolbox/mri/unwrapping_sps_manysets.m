...
%
% Copyright 2007-12-15, Amanda Funai, The University of Michigan
...
for iter = 2:niter
    ...
    for i=1:n
        for j=1:n   
            ...
            grad = grad + wj(:,set) .* mag(:,set) .* (delta(j) - delta(i)) .* sin(s);
            sr = mod(s + pi, 2*pi) - pi;
            denom = denom + wj(:,set) .* mag(:,set) .* (delta(j) - delta(i))^2 .* sinc(sr / pi);
            set = set+1;
        end
    end
