...
%		fld_write(f.pmap, xtrue);
	...
	ymask = ellipse_im(64, 64, [0 0 27 35 0 1], 'oversample', 3) > 0;
	yi_lim = [0 max(abs(yi(:)))];
...
%	xtrue = -20 + 110 * xtrue;
	...
	yi = downsample2(yi, 4);
	ymask = ellipse_im(64, 64, [0 0 21 25 0 1], 'oversample', 3) > 0;
	yi_lim = [0 180];
 end
end
...
if ~isvar('xo'), printm 'xo'
	...
	clf, im pl 3 4; cbar off, im tickoff
...
%	clim = [-40 120];
	...
	t = imdilate(ymask, strel('square', 3)) - ymask;
	im(5, abs(yi) + 0*800*t, '|yi|', yi_lim), cbar

%	ymask = conv2(abs(yi), ones(3)/9, 'same') > (0.1 * max(abs(yi(:))));
%	ymask = imdilate(ymask, strel('disk', 5));
%	ymask = imerode(ymask, strel('disk', 5));
%	im(pl+5, ymask), cbar
%	im(pl+6, abs(yi)/700 + ymask), cbar
	...
	xlabel(sprintf('RMSE = %4.1f Hz', rms(err(ymask))))
...
%	im(1, 0*xtrue, clim, 'None')
	...
	xl = inline('xlabel(sprintf(''NRMS %4.2f%%'', nrms(ft, f0)))');
