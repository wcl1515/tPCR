...
if ~isvar('G'), printm 'G'
	...
	if isvar('f.wtf') && has_mex_jf
		...
		wtfmex(f.wtf, ig.embed(G.arg.G), ...
			ig.nx, ig.ny, sg.nb, sg.na, int32(0)); % save
	end
...
if isvar('f.yi')
	fld_write(f.yi, yi, '-nocheck')
end
if isvar('f.bi')
	fld_write(f.bi, bi, '-nocheck')
end
if isvar('f.ri')
	fld_write(f.ri, ri, '-nocheck')
end
if isvar('f.mask')
	fld_write(f.mask, ig.mask, '-nocheck')
end
