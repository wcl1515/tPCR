...
%
% out
%	ob [nd,np]	np = sum_m ncol(A_m)
%
% Copyright 05-5-12, Jeff Fessler, The University of Michigan
...
% initialize
if ~iscell(blocks)
	error('blocks must be cell array, not "%s"', class(blocks))
end
...
switch arg.type
	...
	otherwise
		error('unknown block type "%s"', arg.type)
end
...
% build Fatrix object
%
ob = Fatrix(arg.dim, arg, 'caller', 'block_fatrix(kron)', ...
