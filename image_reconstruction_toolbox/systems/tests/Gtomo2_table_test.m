...
% paces and adjoint
if 1, printm 'put Gtomo2_table through its paces and test adjoint...'
	...
	Gt2 = Gtomo2_table(sgt, igt, table, 'nthread', 2);
	Gtomo2_test(Gt1, igt.mask, Gt2)
	test_adjoint(Gt1);
	test_adjoint(Gt2);
...
if 1 % look at DD approx
	...
	fe = fd - fs;
	ia = round(sg.na/8);
	ia = imin(abs(sg.ad - 45));
	ia = 1:sg.na;
%	ia = imin(abs(sg.ad - 22.5));
	if im
		plot(ti, fd(:,ia), 'c-', ti, fs(:,ia), 'y-', ti, fe(:,ia), 'g-')
%		im([1*fs, 1*fd, fe]), cbar
		legend('DD', 'Exact', 'Error')
		plot(sg.ad, max(fe, [], 1))
	end
prompt
end
