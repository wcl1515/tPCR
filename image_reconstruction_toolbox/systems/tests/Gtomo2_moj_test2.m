...
%	f.nr = ceil(sqrt(2)/2 * max(ig.nx,ig.ny))*2;
	...
	cpu etic
	Gmoj = Gtomo2_table(sgm, ig, f.table, 'nthread', 1);
	cpu etoc 'Gmoj precompute time:'

	Gpf = rebin_sino([], sgm, sgf, 'ob', 1);
	Gmpf = Gcascade(Gpf, Gmoj);
prompt
end
...
if redo | ~isvar('Gdd'), printm 'setup Gdd'
	cpu etic
% todo: update dd
	Gdd = Gtomo2_dd(ig.mask, sgf.nb, sgf.na, ...
		'chat', 0, ...
		'dis_src_det', sgf.dsd, 'dis_iso_det', sgf.dod, ...
		'offset_s', sgf.offset_s, ...
		'yscale', 1, ...
		'orbit', sgf.orbit, ...
		'orbit_start', sgf.orbit_start, ...	
		'dx', ig.dx, ...
		'ds', sgf.ds, ...
		'strip_width', sgf.ds);
...
if 1, printm 'back'
	...
	yy = zeros(size(ya)); yy(end/4,end/4) = 1; % one ray
