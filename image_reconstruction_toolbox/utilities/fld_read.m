...
%
% options:
%	'raw'	0|1	1: return raw data class, 0: return doubles (default)
%	'slice' slice	specify which slice to read from 3D file. (0 ... nz-1)
...
%
% Copyright 2003-5-13, Jeff Fessler & Rongping Zeng, The University of Michigan
...
if is_external_file
	...
	if ~streq(filetype, 'multi')
		if ~exist(extfile, 'file')
			...
			slash = strfind(fdir, '/');
			if isempty(slash)
				error('cannot find external file %s', extfile)
			end
			fdir = fdir(1:slash(end));
			extfile = [fdir extfile];	% add directory
			if ~exist(extfile, 'file')
				error('no external ref file %s', extfile)
			end
		end
	else
...
if ~streq(filetype, 'multi') % single file reading
%
% reopen file to same position, with appropriate endian too.
%
if is_external_file
	fid = fopen(extfile, 'r', endian);
	if fid == -1, error('cannot open external %s', extfile), end
else
	position = ftell(fid);
	...
	if fid == -1, error('cannot re-open %s', file), end
...
if count ~= prod(rdims)
	disp(rdims)
	fid
	error('file count=%d vs data=%d', count, prod(rdims))
end
if length(rdims) > 1
...
%
% reopen file to the same position, but as vaxd format for coordinates.
% fix: need options to specify the format of the coordinates!
% (The following choices are probably specific to UM RadOnc, not universal.)
%
position = ftell(fid);
fclose(fid);
...
%		printm('Number of coordinate values is 0')
	...
	case 'rectilinear'
		...
		[coord count] = fread(fid, inf, 'float');
		if count ~= cor_num
			error('error in reading coordinate values')
		end

	otherwise
		error('fieldtype "%s" not implement yet!', fieldtype)
end
...
%
% if a line of fld file does not contain a '=' then we use that line as one
% of the filenames
%
...
% setup format as per fread documentation
if arg.raw
	format = ['*' format];
end
...
if ~isempty(arg.slice)
	...
	if fid == -1, error('cannot open external %s', extfile_slice), end
	rdims = dims(1:2);
	...
	data = [];
	count = 0;
	for ii=1:size(extfile_list,1) % read in each file
...
error('could not find %s in header', name)
