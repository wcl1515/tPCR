...
%
% options (new way)
%	'new'			required as 1st arg to signify "new" way
...
%	'raw'		0|1	1: put raw data in file.raw, header in file.fld
...
if length(varargin) && streq(varargin{1}, 'new')
	fld_write_new(file, data, varargin{2:end});
else
	fld_write_old(file, data, varargin{2:end});
end
...
if count ~= numel(data)
	...
	error(sprintf('file count=%d vs data=%d', count, prod(dims)))
end
...
if 1 % test raw/header
	fld_write_test1(file, data, 'new', 'check', 1, 'endian', 'ieee-le', 'raw', 1)
	delete('tmp.raw')
end
...
for ii=1:length(formats)
	format = formats{ii}
	fld_write_test1(file, data, 'new', 'check', 1, 'endian', 'ieee-le', ...
		'type', format)
end
