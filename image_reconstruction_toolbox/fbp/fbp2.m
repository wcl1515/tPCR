...
%
% To use this, you first call it with an empty "geom" argument, with numerous
% parameters that describe the geometry.  The routine returns the initialized
% "geom" structure.  Thereafter, you call this routine with that structure
% (possibly numerous times for the same geometry) to perform FBP reconstruction.
...
%
% Copyright 2005-12-21, Jeff Fessler, The University of Michigan
...
case 'fan'
	geom.arg_fbp_fan = {uint8(ig.mask), ...
		...
		sg.ds, sg.offset, sg.orbit, sg.orbit_start, ...
...
case 'moj'
	if sg.dx == abs(ig.dx)
		...
		warning('mojette sinogram with d=%g vs image with dx=%g', ...
			sg.dx, ig.dx)
	end
...
case 'fan' % fan-beam
	...
	case 'std:mat'
		...
		elseif geom.sg.dfs == 0
			...
			error('bad detector type %s', geom.sg.dfs)
		end
...
%		im(image), cbar
	...
	otherwise
		error('fan not done for type "%s"', arg.type)
	end
...
case 'moj' % mojette, from mojette-rebinned sino
	...
	if geom.sg.dx == abs(geom.ig.dx)
		...
		image = jf_mex('back2', arg1{:}, arg2{:}, single(sino));
		image = image .* geom.ig.mask;
	end
