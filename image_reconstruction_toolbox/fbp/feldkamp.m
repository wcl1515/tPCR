...
img = feldkamp_do(proj, ...
	...
	cg.dsd, cg.dso, cg.dfs, deg2rad(cg.orbit), deg2rad(cg.orbit_start), ...
	ig.mask_or, ig.nz, ig.dx, ig.dy, ig.dz, ...
	[ig.offset_x ig.offset_y ig.offset_z], ...
	arg.window, arg.ia_skip, arg.use_mex);
end % feldkamp()
...
img = feldkamp_do(proj, ...
	...
	deg2rad(arg.orbit), deg2rad(arg.orbit_start), ...
...
function img = feldkamp_do(proj, ...
	...
	dsd, dso, dfs, orbit, orbit_start, ... % radians
	mask, nz, dx, dy, dz, center_xyz, ...
	window, ia_skip, use_mex)
