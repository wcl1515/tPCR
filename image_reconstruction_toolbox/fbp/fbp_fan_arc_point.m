...
if ~isvar('sino')
	...
	% system object
	args = aspire_pair(sg, ig);
	G = Gtomo2_dscmex(args);
...
if ~isvar('xs'), printm 'try various offsets'
	...
	for io=1:length(offsets)
		...
		offset = offsets(io);
		argo = arg_pair('channel_offset', offset);
		argo = strvcat(argo, args);
		G = Gtomo2_dscmex(argo);
