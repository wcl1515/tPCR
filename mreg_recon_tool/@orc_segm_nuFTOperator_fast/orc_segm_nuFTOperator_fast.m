function  A = orc_segm_nuFTOperator_fast(trajectory, imageDim, sensmaps, wmap, dwelltime, Ns, DeltaT,kernel)
...
for k=1:Ns+1
    nstr = nufft_init_fast(trajectory(si{k},:), s.imageDim, s.nufftNeighbors, s.oversampling, s.imageDim/2, kernel);%
    s.interpolation_matrix{k} = nstr.p.arg.G;
    s.interp_filter{k} = spdiags(s.segment_filter{k},-s.segment_index{k}(1)+1,s.trajectory_length,length(s.segment_index{k}))*nstr.p.arg.G;
    s.zrange{k}=nstr.zrange;
end
for k=1:Ns+1
    s.phasemap_coils{k}=repmat(s.wmap(:,:,:,k),[1 1 1 s.numCoils]);
end
s.scaling_factor = nstr.sn; % is the same each time
s.sensmaps_scale = bsxfun(@times,s.scaling_factor,sensmaps);

A = class(s,'orc_segm_nuFTOperator_fast');
