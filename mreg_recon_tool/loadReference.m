...
% adjusts for inplane roation (e.g. change of PE direction)
if ~isfield(twx.hdr.MeasYaps.sSliceArray.asSlice{1}, 'dInPlaneRot') || isempty(twx.hdr.MeasYaps.sSliceArray.asSlice{1}.dInPlaneRot) || strcmp(num2str(twx.hdr.MeasYaps.sSliceArray.asSlice{1}.dInPlaneRot), num2str(0)) || abs(twx.hdr.MeasYaps.sSliceArray.asSlice{1}.dInPlaneRot) < 1e-5  % PE = A >> P
elseif abs(twx.hdr.MeasYaps.sSliceArray.asSlice{1}.dInPlaneRot - pi/2) < 1e-5  % PE = L >> R until VD11
    ref = permute(ref, [2 1 3 4 5]);
    ref = flipdim(ref, 2);
elseif abs(twx.hdr.MeasYaps.sSliceArray.asSlice{1}.dInPlaneRot + pi/2) < 1e-5  % PE = L >> R @ VD13
    ref = permute(ref, [2 1 3 4 5]);
    ref = flipdim(ref, 1);
else
    error('Only Rotations of 0 and plus/minus pi/2 are implemented so far.');
end
