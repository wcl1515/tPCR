function varargout = mreg_recon_tool_new(varargin)
...
%%%%%%%%%%%%
    function load_rawdata_pb_cb(hobj,eventdata,filename)
        ...
        if strcmp(ext,'.mat')
            ...
            D.rawdata = mat2variable(fullfile(pname,fname));
...
%%%%%%%%%%%%
    function load_reference_pb_cb(hobj,eventdata,filename)
        ...
        [~,filename_no_ext,ext] = fileparts(fname);
        if strcmp(ext,'.mat')
        ...
        elseif strcmp(ext,'.dat')
            ...
            drefname = [filename_no_ext, '_dref.mat'];
            if ~exist([pname,'/line_header'],'dir')
                mkdir([pname,'/line_header']);
            end
            if ~exist(fullfile([pname,'/line_header'],drefname),'file')
                [D.reference] = loadReference(fullfile(pname,fname));
                dref=squeeze(D.reference);
                save(fullfile([pname,'/line_header'],drefname),'dref');
            else
                dref=load(fullfile([pname,'/line_header'],drefname));
                D.reference=dref.dref;
            end
...
%%%%%%%%%%%%
    function DORK_cb_cb(hobj, eventdata, val)
        if get(H.DORK_cb,'Value') ~= 1
            ...
            [pname,filename_no_ext,ext] = fileparts(D.recon.recon_details.rawdata_filename);
            fname = [filename_no_ext, '_DORK.mat'];
            if ~exist([pname,'/line_header'],'dir')
                mkdir([pname,'/line_header']);
            end
            if ~exist(fullfile([pname,'/line_header'],fname),'file')
                DORK = DORK_frequency(D.rawdata.rawdata_filename);
                D.recon.recon_details.DORK_frequency=DORK;
                save(fullfile([pname,'/line_header'],fname),'DORK');
            else
                DORK=load(fullfile([pname,'/line_header'],fname));
                D.recon.recon_details.DORK_frequency=DORK.DORK;
            end
...
%%%%%%
%         [pathname,filename_no_ext,ext] = fileparts(D.reference.reference_filename);
%         adrefname = [filename_no_ext, '_adref.mat'];
%         if ~exist([pathname,'/line_header'],'dir')
%             mkdir([pathname,'/line_header']);
%         end
% 
%         if ~exist(fullfile([pathname,'/line_header'],adrefname),'file')
%             adref = resizeReference(D.reference, fov_recon, D.recon.recon_details.recon_resolution, D.rawdata, D.param.sensmode);
%             save(fullfile([pathname,'/line_header'],adrefname),'adref');
%         else
%             adref=load(fullfile([pathname,'/line_header'],adrefname));
%             adref=adref.adref;
%             if D.recon.recon_details.recon_resolution==size(adref.wmap)
%             else
%                 adref = resizeReference(D.reference, fov_recon, D.recon.recon_details.recon_resolution, D.rawdata, D.param.sensmode);
%                 save(fullfile([pathname,'/line_header'],adrefname),'adref');
%             end                
...
%%%%%%%%%%%%
    function local_check
        ...
        if D.check.trajectory_loaded_flag==1 && D.check.reference_loaded_flag==1
            ...
            if strcmp(D.rawdata.sequence(end-3:end), 'diff') 
                ...
                D.recon.recon_details.DeltaT = D.rawdata.te(1) + 2*D.rawdata.trajectIndices(1)*D.rawdata.dwelltime;
            end
        ...
        if D.check.trajectory_loaded_flag==1 && D.check.reference_loaded_flag==1
            if isfield(D.reference, 'dim') && all(D.reference.dim==D.recon.recon_details.recon_resolution) && all(D.reference.fov==D.recon.recon_details.recon_voxel_size.*D.recon.recon_details.recon_resolution) && D.reference.Cor_angle == D.rawdata.Cor_angle && all(D.reference.shift == D.rawdata.shift)
                set(H.adjust_reference_and_trajectory_pb,'BackgroundColor',active_flag_color);
                D.check.adjust_reference_and_trajectory_flag = 1;
            else
                set(H.adjust_reference_and_trajectory_pb,'BackgroundColor',not_active_flag_color);
