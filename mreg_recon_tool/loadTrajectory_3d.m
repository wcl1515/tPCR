function [trajectory, status] = loadTrajectory_3d(filename, pname)
...
while isempty(D)
    ...
    elseif ~isempty(strfind(tline,'Field_Of_View_[mm]'))
        fov = [str2num(S{2}) str2num(S{3}) str2num(S{4})]/1e3;
    elseif ~isempty(strfind(tline,'Base_Resolution'))
        N = [str2num(S{2}) str2num(S{3}) str2num(S{4})];
...
G = G_up;
for n=1:NumOfElem
    Gout{n}=G((n-1)*ElementLength*10+1:n*ElementLength*10,:);
    Kout{n} = (cumsum(Gout{n},1)*(max_grad/1000)*SYS.GRT_SI/10)*SYS.GAMMA_SI/(2*pi);
    Kout{n}(:,2:3) = - Kout{n}(:,2:3);
    Kout{n}(:,1)=(Kout{n}(:,1)/kmax(1))*pi;
    Kout{n}(:,2)=(Kout{n}(:,2)/kmax(2))*pi;
    Kout{n}(:,3)=(Kout{n}(:,3)/kmax(3))*pi;
