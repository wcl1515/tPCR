function  A = orc_segm_nuFTOperator_cartesian(trajectory, imageDim, sensmaps, wmap, dwelltime, Ns, DeltaT)
...
for k=1:Ns+1
    tra=trajectory(si{k},:);
    s.tra_idx{k}(:,1)=mod(round(((tra(:,1)/2/pi)+0.5)*imageDim(1)),imageDim(1))+1;
    s.tra_idx{k}(:,2)=mod(round(((tra(:,2)/2/pi)+0.5)*imageDim(2)),imageDim(2))+1;
end    

A = class(s,'orc_segm_nuFTOperator_cartesian');
