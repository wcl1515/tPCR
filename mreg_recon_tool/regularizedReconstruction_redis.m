function [z, z_iter] = regularizedReconstruction_redis(A,b,varargin)
...
if linear_flag==0 % use nonlinear conjugate gradient
    ...
    if nargout==1
        [z] = nonlinearConjugateGradient_redis(f,df,z0,maxit,cg_method,verbose_flag,reference_norm);
    else
        [z, z_iter] = nonlinearConjugateGradient_redis(f,df,z0,maxit,cg_method,verbose_flag,reference_norm);
    end
    ...
    if nargout==1
        z = conjugateGradient_redis(S,q,maxit,z0,verbose_flag,newfigure);
    else
        [z,z_iter] = conjugateGradient_redis(S,q,maxit,z0,verbose_flag,newfigure);
    end
