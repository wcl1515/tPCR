function  A = nuFTOperator_multi(trajectory, imageDim, sensmaps, os, neighbors, kernel)
...
if nargin==0 % default constructor
    ...
    s.timeframe=length(trajectory);
    s.imageDim = imageDim;
    % By default the operator is not adjoint, to get the adjoint, just call
    % A'
    s.adjoint = 0;
    s.trajectory_length = size(trajectory{1},1);
    ...
    for n=1:length(trajectory0)
        ...
        s.nufftStruct{n} = nufft_init(trajectory, imageDim, s.nufftNeighbors, round(os*imageDim), ceil(imageDim/2), kernel);
    end
end
A = class(s,'nuFTOperator_multi');
