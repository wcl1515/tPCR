function  A = orc_segm_nuFTOperator_multi(trajectory, imageDim, sensmaps, wmap,  dwelltime, Ns, DeltaT,kernel)
...
%         nstr = nufft_initnew(trajectory(si{k},:), s.imageDim, s.nufftNeighbors, s.oversampling,trasign(k), s.imageDim/2, 'kaiser');
        nstr = nufft_init(trajectory(si{k},:), s.imageDim, s.nufftNeighbors, s.oversampling, s.imageDim/2, kernel);
