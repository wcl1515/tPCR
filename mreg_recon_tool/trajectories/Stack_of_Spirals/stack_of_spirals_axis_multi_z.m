function T = stack_of_spirals_axis_multi_z(R,fov,resolution,Nz)
...
kz1=kz(1:2:end);
kr_max1=kr_max(1:2:end);
kz2=kz(2:2:end);kz2=kz2(end:-1:1);
kr_max2=kr_max(2:2:end);kr_max2=kr_max2(end:-1:1);
...
for element = 1:length(kz1)
    Rmin = (Rrad_min + abs(kz1(element)/k_max(3)) * (Rrad_max - Rrad_min));
    Rmax = Rrad_max;
...
for element = 1:length(kz2)
    Rmin = (Rrad_min + abs(kz2(element)/k_max(3)) * (Rrad_max - Rrad_min));
    Rmax = Rrad_max;
    T(element, iz, 1) = single_element_spiral(kz2(element), kr_max2(element), Rmin, Rmax, fov(1), in_out, SYSTEM);
    if element > 1
        last2 = T(element-1,iz,1).K(end-1:end,1)+ 1i * T(element-1,iz,1).K(end-1:end,2);
        first2 = T(element,iz,1).K(1:2,1)+ 1i* T(element,iz,1).K(1:2,2);
        alpha = angle(diff(last2,1,1)) - angle(diff(first2,1,1));
...
Tc = T(1,1);
for element=2:length(kz1)
    Tc = trajectStruct_connect(Tc,T(element,1), 'rip');
end
T_temp(1)=Tc;
Tc = T(1,2);
for element=2:length(kz2)
    Tc = trajectStruct_connect(Tc,T(element,2), 'rip');
end
T_temp(2)=Tc;
if Nz==6
    T_temp(3)=trajectStruct_rotate(T_temp(1),pi/2,[1 0 0]);
    T_temp(4)=trajectStruct_rotate(T_temp(2),pi/2,[1 0 0]);
    T_temp(5)=trajectStruct_rotate(T_temp(1),pi/2,[0 1 0]);
    T_temp(6)=trajectStruct_rotate(T_temp(2),pi/2,[0 1 0]);
end
T=T_temp;
