function T = stack_of_spirals_retrack(R,Nradial,Nz,fov,resolution, rotat)
...
T(1).K(end+1:end+50,:)=0;
[~, te]    = min(makesos(T(1).K(200:end-200,:), 2));
te=te+200;
if te>length(T(1).K)/2
    T(1).K(end+1:end+(2*te-length(T(1).K)),:)=0;
else
    temp=T(1).K;
    T(1).K(1:(length(T(1).K)-2*te),:)=0;
    T(1).K((length(T(1).K)-2*te+1):2*(length(T(1).K)-te),:)=temp;
end
T(2)=T(1);
T(2).K=T(1).K(end:-1:1,:);
...
for i=1:length(T)
    ...
    [~, te]    = min(makesos(T(i).K(1000:end-1000,:), 2));  % The Echotime of the first trajectory is taken. Hopefully they are all similar...
    T(i).TE    = (te + 1000) * 10; % [us]
end
